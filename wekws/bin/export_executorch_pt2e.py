...
def _lower_to_executorch(exported_program, use_xnnpack: bool):
...
def _export_int8_pt2e(wrapper: torch.nn.Module,
                      example_inputs,
                      configs,
...
def main():
    ...
    if not output_model:
        output_model = os.path.join(
            checkpoint_dir,
            '{}_executorch_{}.pte'.format(checkpoint_basename, args.quant_type))
    ...
    logging.info('Use XNNPACK lowering: %s', use_xnnpack)

    if args.quant_type == 'fp32':
        exported = torch.export.export(wrapper, example_inputs)
    else:
        exported = _export_int8_pt2e(wrapper, example_inputs, configs, args)

    exec_prog = _lower_to_executorch(exported, use_xnnpack=use_xnnpack)
